    ...
    def _init_torchmetrics(self, prefix):
        ...
        for name in self.torchmetric_names:
            if name in ['AUROC', 'StatScores', 'Precision', 'Recall', 'F1', 'F1Score']:
                self._tracked_torchmetrics[prefix][name] = getattr(tm, name)(task='multilabel', average='macro', num_labels=8, compute_on_step=False).to('cuda')
            elif '@' in name:
                k = int(name.split('@')[1])
                mname = name.split('@')[0]
    ...
    def torchmetrics(self, x, y, prefix):
        ...
        for name in self.torchmetric_names:
            ...
            if len(x.shape) > 2:
                ...
                self._tracked_torchmetrics[prefix][name].update(x.squeeze(-1), y.to(torch.long))
                continue
            self._tracked_torchmetrics[prefix][name].update(x, y.to(torch.long))
